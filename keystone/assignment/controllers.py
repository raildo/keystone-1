...
class RoleAssignmentV3(controller.V3Controller):
    ...
    def list_role_assignments(self, context, filters):
        ...
        if ('effective' in query_string and
                self.query_filter_is_true(query_string['effective'])):

            formatted_refs = self._expand_indirect_assignments(context,
                                                               formatted_refs)
        elif not (
            'scope.OS-INHERIT:inherited_to' in query_string and
                query_string['scope.OS-INHERIT:inherited_to'] == 'projects'):
            # Remove any role assignments with project scope and
            # inherited_to_projects flag
            formatted_refs = (
                [x for x in formatted_refs
                    if not ('project' in x['scope'] and
                            'OS-INHERIT:inherited_to' in x['scope'])])
