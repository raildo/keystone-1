...
class IdentityTests(object):
    ...
    def test_get_projects_hierarchy_subtree(self):
        projects_hierarchy = self._create_projects_hierarchy(3)
        project1 = projects_hierarchy[0]
        project2 = projects_hierarchy[1]
        leaf_project = projects_hierarchy[2]
        other_leaf_project = {'id': uuid.uuid4().hex,
                    'description': '',
                    'domain_id': DEFAULT_DOMAIN_ID,
                    'enabled': True,
                    'name': uuid.uuid4().hex,
                    'parent_id': project2['id']}
        self.assignment_api.create_project(other_leaf_project['id'], other_leaf_project)

        subtree = self.assignment_api.get_project_hierarchy_subtree(project1['id'])

        self.assertEqual(None, subtree.get(leaf_project['id']))
        self.assertIn(leaf_project['id'], subtree.get(project2['id']))
        self.assertIn(other_leaf_project['id'], subtree.get(project2['id']))



    def test_get_projects_hierarchy_parents(self):
        projects_hierarchy = self._create_projects_hierarchy(3)
        project1 = projects_hierarchy[0]
        project2 = projects_hierarchy[1]
        project3 = projects_hierarchy[2]
        parents = self.assignment_api.get_project_hierarchy_parents(project3['id'])
    ...
    def test_list_projects_in_subtree(self):
        projects_hierarchy = self._create_projects_hierarchy(2)
        project1 = projects_hierarchy[0]
        project2 = projects_hierarchy[1]
        project3 = projects_hierarchy[2]
